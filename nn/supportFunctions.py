...
#Make Toeplitz Design Matrix from input vector
...
def checkTimeAlignmentAndPlot(input, output, exampleSize = 128, maxOffset = 200, stepSize = 5, \
    ...
    #Measure correlation and ols (ordinary least squares) performance for various time-alignments.
    ...
    #Check time-alignment by training linear model:
    ...
    #Check time-alignemnent by measuring correlation:
	offsets, correlations = timeShiftAndCorrelate(maxOffset = maxOffset, stepSize = stepSize, \
	...
	if plot:
		...
		pyplot.ylabel('MSE')
		pyplot.legend(['train Mic', 'test Mic'], loc=1)
		...
		pyplot.legend(['Correlation Mic'], loc=1)
